...
export const validateAndRegisterVoterInFirestore = async (
  email: string,
  ...
  try {
    ...
    const normName = cleanName.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ');
    ...
    if (snap.exists()) {
      electionData = snap.data();
      if (Array.isArray(electionData.voters)) {
        voters = [...electionData.voters];
      }
    }

    const emailMatch = voters.find(
      (v) => v.email && v.email.trim().toLowerCase() === cleanEmail
    );
    const nameMatch = voters.find(
      (v) =>
        normName.length > 2 &&
        v.name &&
        v.name.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ') === normName
    );
    const idMatch = voters.find(
      (v) => v.id && v.id.trim().toUpperCase() === cleanStudentId
    );
    ...
    if (nameMatch) {
      if (
        ...
        (nameMatch.email && nameMatch.email.trim().toLowerCase() !== cleanEmail)
      ) {
        nameConflict = true;
      }
    }
    ...
    if (emailMatch) {
      if (
        ...
        emailMatch.id !== nameMatch.id ||
        (emailMatch.name &&
          emailMatch.name.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ') !== normName &&
    ...
    // Check if voter or matching account already voted
    const matchingVoted = voters.find(
      (v) =>
        v.hasVoted &&
        ((v.email && cleanEmail && v.email.trim().toLowerCase() === cleanEmail) ||
          (normName.length > 2 &&
            v.name &&
            v.name.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ') === normName) ||
          (v.id && cleanStudentId && v.id.trim().toUpperCase() === cleanStudentId))
    );
    ...
    if (voter) {
      ...
      voter.yearLevel = yearLevel as any;
      if (matchingVoted) {
        voter.hasVoted = true;
    ...
    console.warn('validateAndRegisterVoterInFirestore note:', err);
    return {
      success: true,
      voter: {
        id: studentNumber,
